...
def fig_pressure_all_stations(opal_prod, pub_dir, plt):
    ...
    for idx, (gs, label) in enumerate(zip(stations, labels)):
        ...
        ax.plot(t_opal, p_opal, '-', color=OPAL_PROD, label='OPAL V24')
...
def fig_pressure_early_time(opal_prod, pub_dir, plt):
    ...
    for idx, (gs, label) in enumerate(zip(early_stations, labels)):
        ...
        t_opal, p_opal = get_opal_pressure_at_gs(opal_prod, gs)
        mask = t_opal <= 0.05
        ax.plot(t_opal[mask], p_opal[mask], '-', color=OPAL_PROD, label='OPAL V24')
...
def fig_void_fraction_gs5(opal_prod, opal_base, pub_dir, plt):
    ...
    ax.plot(t_prod, a_prod, '-', color=OPAL_PROD, linewidth=1.5, label='OPAL V24')
...
def fig_mape_comparison(opal_prod, pub_dir, plt):
    """Figure 5: MAPE comparison bar chart (OPAL V24 vs RELAP5-3D)."""
    print("\n--- Figure 5: MAPE comparison ---")

    import matplotlib.patches as mpatches

    # Compute OPAL V24 per-station MAPE
    opal_mape = {}
    for gs in station_order:
        t_opal, p_opal = get_opal_pressure_at_gs(opal_prod, gs)
        t_exp, p_exp = load_exp_pressure(gs)
    ...
    ax.bar(x - w / 2, opal_vals, w, color=OPAL_PROD, alpha=0.85, label='OPAL V24')
...
def update_tables(pub_dir, opal_mape=None, relap_mape=None,
    ...
    lines.append("## Table 1: Per-Station MAPE -- OPAL V24 (N=24, tau_mix=450us)\n")
    ...
    # Table 3: Code comparison OPAL V24 vs RELAP5-3D
    lines.append("\n## Table 3: Code Comparison -- Per-Station MAPE [%]\n")
    lines.append("| Station | x [m] | OPAL V24 | RELAP5-3D | Delta |")
    lines.append("|---|---:|---:|---:|---:|")
    for gs in station_order:
    ...
    if prod_data:
        lines.append(f"| OPAL V24 (production) | {prod_data.get('void_mae_gs5', 'N/A')} "
    ...
    lines.append("\n## Table 7: Simulation Setup\n")
    lines.append("| Parameter | OPAL V24 | RELAP5-3D |")
    lines.append("|---|---|---|")
...
def main():
    ...
    print("=" * 70)
    print("OPAL V24 Publication Generator")
