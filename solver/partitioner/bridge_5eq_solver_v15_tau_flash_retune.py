"""
bridge_5eq_solver_v15_tau_flash_retune.py -- Variant 15: tau_flash runtime override.

Based on V11 (2x2 block Thomas with A12 time-constant moderation). Adds the
ability to override the Modelica parameter `pipe.tau_flash` at runtime from
Python, without recompiling the bridge .so.

Motivation: V11 achieves correct void onset (10ms) with tau_mix=5e-4, but
VoidMAE=0.298. The flashing relaxation time tau_flash=0.025 in the Modelica
model was calibrated under the stalled regime (140ms onset) where its effect
was negligible. With correct onset now active, tau_flash directly controls
void growth speed. This variant makes tau_flash easily tunable from Python
for parameter sweeps.

Implementation: after bridge.set_params_from_spec() writes all parameters,
V15 calls opal_bridge_set_param(index, value) to overwrite the single
tau_flash entry. This uses the existing C API -- no array rebuild needed.
...
class BridgeDriftFluxSolver:
    ...
    and runtime tau_flash override.

    Based on V11 (block Thomas + A12 moderation). Adds tau_flash_override to
    retune the flashing relaxation timescale from Python without recompiling
    the Modelica bridge .so. This is useful for parameter sweeps when the
    Modelica value was calibrated under a different solver regime.
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
        ...
        Args:
            ...
            tau_flash_override: if not None, overrides the Modelica parameter
                pipe.tau_flash in the bridge after set_params_from_spec. Units: [s].
                The Modelica default is 0.025s (calibrated under stalled onset).
                Typical retune range: 0.001 to 0.01.
        """
        ...
        # V15: override tau_flash in the bridge parameter array.
        # Uses opal_bridge_set_param(index, value) to overwrite a single entry
        # after set_params_from_spec has written the full array.
        if tau_flash_override is not None:
            self._override_bridge_param('pipe.tau_flash', tau_flash_override)

    def _override_bridge_param(self, short_name, value):
        """Override a single parameter in the bridge after set_params_from_spec.

        Args:
            short_name: parameter name without prefix (e.g., 'pipe.tau_flash').
                        The bridge prefix is prepended automatically if needed.
            value: new parameter value (float).

        Raises:
            KeyError: if the parameter is not found in the bridge info.
        """
        import ctypes

        # Try with bridge prefix first, then bare name
        prefix = self.bridge.prefix
        candidates = [
            f'{prefix}.tau_flash' if short_name == 'pipe.tau_flash' else f'{prefix}.{short_name}',
            short_name,
        ]
        ...
        for name in candidates:
            if name in self.bridge.info.parameters:
                pinfo = self.bridge.info.parameters[name]
                resolved_name = name
                break

        if pinfo is None:
            raise KeyError(
                f"Parameter '{short_name}' not found in bridge. "
                f"Tried: {candidates}. "
                f"Available parameters with 'tau': "
                f"{[k for k in self.bridge.info.parameters if 'tau' in k.lower()]}"
            )

        self.bridge.lib.opal_bridge_set_param(
            ctypes.c_int(pinfo.index), ctypes.c_double(float(value))
        )
