"""
bridge_5eq_solver_v9_newton.py -- Variant 9: Newton iteration within block Thomas.

Extends V5 (2x2 block Thomas) with Newton-like iterations within each timestep.
V5 achieves correct void onset (15ms) but 147% pressure MAPE because the
off-diagonal coupling (A12*A21) exceeds the diagonal product (A11*A22) by ~1000x
at onset. A single block Thomas pass cannot converge this massive cross-coupling.

The fix (following RELAP5): iterate the block Thomas solve 2-4 times per timestep.
Each iteration re-evaluates the bridge at the current (p, alpha) state, getting
updated drho_dp, Gamma, rho_l, rho_v, etc. The key insight:

  - Iteration 0: use PHASIC drho_dp for A11 (allows void onset detection)
  - Iterations 1+: use h_mix drho_dp from re-evaluated bridge (correct wave speed)

This naturally transitions from onset-friendly to wave-speed-correct within the
same timestep, resolving the A11 dilemma that plagued V5.
...
class BridgeDriftFluxSolver:
    """5-equation drift-flux solver with Newton-iterated block Thomas solve.

    Physics (properties, closures, friction multiplier, interfacial transfer)
    from OM-generated C. Numerics (block tridiagonal solve, Newton iteration,
    momentum, phasic energy) from Python.

    Each timestep performs n_newton iterations of the 2x2 block Thomas solve.
    Iteration 0 uses phasic compressibility (onset-friendly). Subsequent
    iterations re-evaluate the bridge at the updated (p, alpha) state and use
    h_mix compressibility (wave-speed-correct). This converges the strong
    pressure-void coupling that a single pass cannot resolve.
    """
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
        ...
        Args:
            ...
            use_block_coupling: accepted for API compatibility but ignored --
                block coupling is always active in V9.
            corrector_steps: accepted for API compatibility but ignored --
                Newton iteration replaces corrector.
            n_newton: number of Newton iterations per timestep (default 3).
                1 = identical to V5 (single block Thomas pass).
                2-4 = recommended for strongly coupled problems.
        """
        self.reconstruction = reconstruction
        # V9: block coupling is always active; Newton iteration replaces corrector
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        """One semi-implicit timestep with Newton-iterated block Thomas solve.

        Each Newton iteration re-evaluates the bridge at the current (p, alpha)
        state and re-solves the 2x2 block tridiagonal system. The delta is always
        relative to start-of-step values (p_old, alpha_old), not the previous
        iteration. This ensures convergence rather than oscillation.

        Iteration 0 uses phasic drho_dp for A11 (onset-friendly).
        Iterations 1+ use h_mix drho_dp from re-evaluated bridge (wave-speed-correct).
        ...
        # -- Friction with implicit resistance (frozen for all Newton iterations) --
        ...
        # ====================================================================
        # p_old and alpha_old are FIXED throughout all iterations.
        # Each iteration solves for delta from start-of-step.
        # Friction and beta_eff are frozen from initial evaluation.
        # Momentum is updated after each iteration so the block RHS
        # sees the current mdot (needed for mass flux residual).

        for newton_iter in range(self.n_newton):
            # ----------------------------------------------------------------
            # Re-evaluate bridge at current (p, alpha) for iterations 1+
            # ----------------------------------------------------------------
            if newton_iter > 0:
                ...
                # Updated physics from re-evaluated bridge
                drho_dp = self.bridge.get('drho_dp')
                rho_l = self.bridge.get('rho_l')
                rho_v = self.bridge.get('rho_v')
                Gamma = self.bridge.get('Gamma')
                ...
                # Updated phasic compressibilities
                if self.bridge.has('drho_l_dp') and self.bridge.has('drho_v_dp'):
                    drho_l_dp = self.bridge.get('drho_l_dp')
                    drho_v_dp = self.bridge.get('drho_v_dp')

                # Updated drift-flux phasic flows
                if has_drift_flux:
                    mdot_v_face = self.bridge.get('mdot_v')
                    mdot_l_face = self.bridge.get('mdot_l')
            ...
            #   [A21  A22] [delta_alpha] = [R2]
            #
            # Row 1 (mixture mass): V/dt * drho_A11 * dp + V/dt * (rho_v - rho_l) * dalpha
            ...
            for i in range(N):
                ...
                # A11 compressibility: iteration-dependent strategy
                #   Iteration 0: phasic drho_dp (onset-friendly, allows void detection)
                #   Iterations 1+: h_mix drho_dp from bridge (correct wave speed)
                if newton_iter == 0:
            ...
            # Apply solution: delta is relative to start-of-step (p_old, alpha_old)
            for i in range(N):
                delta_p = x[i][0]
                delta_alpha = x[i][1]
            ...
            # -- Update momentum (with implicit friction) --
            # Momentum uses frozen friction/beta_eff but current p from this iteration.
            mdot[0] = 0.0  # wall BC
            for i in range(1, N):
        ...
        # ====================================================================
        # PHASIC ENERGY UPDATE (once, after all Newton iterations)
        # ====================================================================
        # Energy uses final p, alpha, mdot from Newton iterations.
        # Enthalpy advection uses h_l_old/h_v_old to prevent directional bias.
        # rho_l, rho_v, q_i_l, q_i_v, Gamma etc. are from the last bridge
        # evaluation (final Newton iteration), which is the most consistent state.
        dp_dt = (p - p_old) / dt
