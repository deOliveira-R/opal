"""
bridge_5eq_solver_v1_rv_fix.py — V1: rho_v linearization fix.

Fix: use linearized rho_v at NEW pressure for the void fraction division.
The base solver uses rho_v at OLD pressure, underestimating void growth during
rapid depressurization. rho_v_new = rho_v + drho_v_dp * (p_new - p_old).
...
class BridgeDriftFluxSolver:
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        # V1: Always get drho_v_dp for linearized rho_v at new pressure
        drho_v_dp = (self.bridge.get('drho_v_dp')
                     if self.bridge.has('drho_v_dp') else np.zeros(N))
        ...
        if True:
            ...
            for i in range(N):
                ...
                # V1: linearize rho_v at NEW pressure for correct void amplification
