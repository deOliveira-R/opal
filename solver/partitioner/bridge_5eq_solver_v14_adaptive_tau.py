"""
bridge_5eq_solver_v14_adaptive_tau.py -- Variant 14: Adaptive per-cell tau_mix.

Based on V11 (2x2 block Thomas with A12 time-constant moderation). V11 uses a
constant tau_mix for all cells. The tau_mix sweep revealed a tension:
- Small tau_mix (2.5e-4): VoidMAE=0.111 (RELAP5 level) but MAPE=76%
- Large tau_mix (4.0e-4): MAPE=28.2% but VoidMAE=0.223

The insight: tight void coupling (small tau_mix) is needed at flashing onset
(alpha near 0) where the void must grow quickly, but stable pressure response
(large tau_mix) is needed in established two-phase (alpha > ~0.02) where the
massive A12 term can amplify small void oscillations into pressure noise.

V14 makes tau_mix adaptive per cell based on the local void fraction:

    blend = min(alpha / alpha_transition, 1.0)
    tau_mix_eff = tau_min + (tau_max - tau_min) * blend
    A12 = V/dt * (rho_v - rho_l) / (1 + tau_mix_eff / dt)

At onset (alpha -> 0): tau_mix_eff = tau_min  (tight coupling, fast void growth)
At established two-phase: tau_mix_eff = tau_max  (relaxed coupling, stable pressure)
The transition is a smooth linear ramp over [0, alpha_transition].
...
class BridgeDriftFluxSolver:
    """5-equation drift-flux solver with 2x2 block Thomas and adaptive A12 moderation.

    Based on V11 block Thomas. Replaces the constant tau_mix with a per-cell
    adaptive tau_mix_eff that depends on the local void fraction alpha:

        blend = min(alpha / alpha_transition, 1.0)
        tau_mix_eff = tau_min + (tau_max - tau_min) * blend

    At flashing onset (alpha ~ 0), tau_mix_eff is small (tight coupling) so the
    void fraction can grow quickly. In established two-phase (alpha > alpha_transition),
    tau_mix_eff is large (relaxed coupling) so the massive A12 = V/dt*(rho_v - rho_l)
    term doesn't amplify small void oscillations into pressure noise.
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
                 ...
                 corrector_steps=0,
                 tau_mix_min=1e-4, tau_mix_max=5e-4, alpha_transition=0.02):
        """
        Args:
            ...
            tau_mix_min: minimum thermal relaxation timescale [s] for A12 moderation.
                Used at alpha=0 (flashing onset). Tight coupling for void accuracy.
                Default 1e-4 s.
            tau_mix_max: maximum thermal relaxation timescale [s] for A12 moderation.
                Used at alpha >= alpha_transition (established two-phase). Relaxed
                coupling for pressure stability. Default 5e-4 s.
            alpha_transition: void fraction at which tau_mix_eff reaches tau_max.
                The transition is a smooth linear ramp from tau_min at alpha=0
                to tau_max at alpha=alpha_transition. Default 0.02.
        """
        self.reconstruction = reconstruction
        # V5: block coupling is always active; corrector is unnecessary
        self.use_block_coupling = True
        self.corrector_steps = 0
        self.tau_mix_min = tau_mix_min
        self.tau_mix_max = tau_mix_max
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        A12 moderation uses adaptive per-cell tau_mix_eff based on local void:
        tau_mix_eff = tau_min + (tau_max - tau_min) * min(alpha / alpha_transition, 1)
        ...
        # ====================================================================
        # ADAPTIVE tau_mix PER CELL
        # ====================================================================
        # tau_mix_eff grows from tau_min at alpha=0 to tau_max at alpha >= alpha_transition.
        # This gives tight void coupling at flashing onset (good void accuracy)
        # and relaxed coupling in established two-phase (stable pressure).
        tau_min = self.tau_mix_min
        tau_max = self.tau_mix_max
        alpha_trans = self.alpha_transition
        ...
        for i in range(N):
            ...
            # Moderated by adaptive thermal relaxation timescale tau_mix_eff.
            # At alpha=0: tau_mix_eff=tau_min (tight coupling, fast void growth)
            # At alpha>=alpha_trans: tau_mix_eff=tau_max (relaxed, stable pressure)
            blend = min(al / alpha_trans, 1.0) if alpha_trans > 0 else 1.0
            tau_mix_eff = tau_min + (tau_max - tau_min) * blend
