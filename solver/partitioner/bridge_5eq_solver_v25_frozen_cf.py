"""
bridge_5eq_solver_v25_frozen_cf.py -- Variant 25: V24 + frozen critical flow sound speed.

Based on V24 (break form-loss + V11 block Thomas + isentropic A11).
Adds: frozen mixture sound speed for the critical flow HEM branch.

Bug in CriticalFlow.mo: The HEM branch uses drho_dp|_h at h_mix (equilibrium
compressibility ~5 Pa^-1), giving c_HEM ≈ 0.016 m/s — absurdly low. Once the
outlet cell reaches two-phase (x_e > x_ne), critical flow drops to near-zero
and the break becomes momentum-limited rather than choke-limited.

Fix: Compute frozen mixture sound speed from isentropic phasic compressibilities
(already in bridge: drho_l_dp_s, drho_v_dp_s). Use this for a solver-side
critical flow correction when the Modelica HEM branch is active.

  c_frozen² = 1 / (rho_m * [(1-α)/( ρ_l·c_l²) + α/(ρ_v·c_v²)])
  G_crit_frozen = rho_m * c_frozen
  mdot_crit = max(mdot_crit_bridge, C_d * A * G_crit_frozen)
...
class BridgeDriftFluxSolver:
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
                 ...
                 use_isentropic_a11=True, break_form_loss=True,
                 use_frozen_cf=True):
        """
        Args:
            ...
            break_form_loss: Add localized form loss K=(1/C_d²-1) at outlet
                face to enforce break opening ramp through momentum equation.
                Default True (V24 behavior). False recovers V11.
            use_frozen_cf: Use frozen mixture sound speed for the critical flow
                HEM branch instead of equilibrium c_HEM. Fixes the bug where
                c_HEM ≈ 0.016 m/s from h_mix drho_dp. Default True (V25).
        """
        ...
        # Break form loss: localized K=(1/C_d²-1) at outlet face
        self.break_form_loss = break_form_loss
        # Frozen critical flow sound speed (V25)
        self.use_frozen_cf = use_frozen_cf
        # Find C_d_eff variable index for direct read from bridge
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        # -- Frozen critical flow sound speed (V25) --
        # CriticalFlow.mo uses c_HEM = 1/sqrt(rho*drho_dp_h) with h_mix
        # equilibrium compressibility. In two-phase, this gives c ~ 0.016 m/s
        # (absurd). The frozen mixture sound speed uses isentropic phasic
        # compressibilities: 1/(rho*c²) = (1-α)/(ρ_l*c_l²) + α/(ρ_v*c_v²).
        # This gives physically correct c ~ 50-400 m/s.
        if self.use_frozen_cf and self.use_critical_flow:
            al_out = alpha_old[N - 1]
            if al_out > 1e-6:
                # Frozen mixture compressibility at outlet cell
                drho_frozen = ((1 - al_out) * drho_l_dp_s[N - 1]
                               + al_out * drho_v_dp_s[N - 1])
                rho_m_out = ((1 - al_out) * rho_l[N - 1]
                             + al_out * rho_v[N - 1])
                if drho_frozen > 0 and rho_m_out > 0.01:
                    c_frozen = np.sqrt(1.0 / (rho_m_out * drho_frozen))
                    c_frozen = max(c_frozen, 10.0)  # floor
                    G_frozen = rho_m_out * c_frozen
                    # Get C_d for the frozen flux
                    C_d_val = 1.0
                    if self._cd_eff_idx is not None:
                        C_d_val = self.bridge.lib.opal_bridge_get_var(
                            self._cd_eff_idx)
                        if not np.isfinite(C_d_val) or C_d_val < 1e-4:
                            C_d_val = 1e-4
                    mdot_crit_frozen = C_d_val * self.A_flow * G_frozen
                    # Use max: frozen CF provides a FLOOR for mdot_crit
                    # (the Modelica HEM value may be absurdly low)
                    mdot_crit = max(mdot_crit, mdot_crit_frozen)
        ...
        # -- Break form loss at outlet face (V24) --
        if self.break_form_loss:
            # Read C_d from bridge (Modelica RampedBreak computes it)
            C_d_current = 1.0
            if self._cd_eff_idx is not None:
            ...
            if K_break > 0 and rho_face[N] > 0.01 and np.isfinite(mdot_old[N]):
                fric_break = (K_break * abs(mdot_old[N]) * mdot_old[N]
                              / (rho_face[N] * self.A_flow**2))
                if np.isfinite(fric_break):
                    fric[N] += fric_break
                sigma_break = (2 * dt * K_break * abs(mdot_old[N])
                               / (rho_face[N] * self.A_flow**2))
