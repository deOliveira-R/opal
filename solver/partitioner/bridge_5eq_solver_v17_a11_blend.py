"""
bridge_5eq_solver_v17_a11_blend.py -- Variant 17: A11 alpha-blend on V11 base.

Combines V11's A12 time-constant moderation (tau_mix) with V6's alpha-blended
A11 compressibility. At low void (onset), A11 uses phasic drho_dp for correct
nucleation. In established two-phase, A11 transitions to h_mix drho_dp for
correct wave speed damping.

V11 used pure phasic A11 everywhere, which gives correct onset but may lack
wave speed control in established two-phase. V6 blended A11 but without A12
moderation, causing excessive cross-coupling. V17 combines both fixes:

    A11 blend: phasic (alpha < alpha_onset) -> h_mix (alpha > alpha_wave)
    A12 moderation: V/dt * (rho_v - rho_l) / (1 + tau_mix / dt)  [from V11]
...
class BridgeDriftFluxSolver:
    """5-equation drift-flux solver: V11 A12 moderation + V6 A11 alpha-blend.

    Combines two complementary techniques for the 2x2 block Thomas solve:
    - A11 alpha-blend (from V6): phasic drho_dp at onset transitions to h_mix
      drho_dp in established two-phase, giving correct wave speed control.
    - A12 tau_mix moderation (from V11): prevents the ~-2.5e6 A12 coupling
      from creating excessive pressure feedback during rapid depressurization.
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
                 ...
                 alpha_onset=0.005, alpha_wave=0.05):
        """
        Args:
            ...
            alpha_onset: void fraction below which A11 uses pure phasic drho_dp.
                Phasic compressibility allows rapid nucleation at onset.
                Default 0.005 (0.5% void).
            alpha_wave: void fraction above which A11 uses pure h_mix drho_dp.
                h_mix compressibility provides correct wave speed in established
                two-phase. Default 0.05 (5% void). Linear blend between thresholds.
        """
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        for i in range(N):
            ...
            # Mechanical (phasic) compressibility: how mixture density changes
            # with p at fixed void fraction and fixed phasic enthalpies
            drho_mech = (1 - al) * drho_l_dp[i] + al * drho_v_dp[i]

            # Alpha-blend A11: phasic at onset -> h_mix in established two-phase.
            # Phasic drho_dp allows rapid nucleation (correct onset timing).
            # h_mix drho_dp includes thermal compressibility via saturation curve
            # shift, providing correct wave speed damping in two-phase.
            ao = self.alpha_onset
            aw = self.alpha_wave
            if al <= ao:
                drho_A11 = drho_mech
            elif al >= aw:
                drho_A11 = drho_dp[i]  # h_mix from bridge
            else:
                blend = (al - ao) / (aw - ao)
                drho_A11 = (1.0 - blend) * drho_mech + blend * drho_dp[i]
