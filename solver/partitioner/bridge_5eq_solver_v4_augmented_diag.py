"""
bridge_5eq_solver_v4_augmented_diag.py — Variant 4: Augmented diagonal with smooth
alpha scaling.

RELAP5-inspired pressure diagonal: phasic mechanical compressibility PLUS a
Gamma-linearized void-density coupling term that grows smoothly with evaporation
rate. This replaces the h_mix drho_dp step function (2400x jump at saturation)
with a physically motivated augmentation derived from the Clausius-Clapeyron
relation.

The augmented diagonal is:
    drho_eff = drho_mech + drho_void_coupling
where:
    drho_mech = (1-alpha)*drho_l/dp + alpha*drho_v/dp   (phasic mechanical, ~5e-7)
    drho_void_coupling = -(rho_l - rho_v)/rho_v * dGamma/dp * dt
                        (from linearizing the void equation's Gamma-pressure feedback)

The dGamma/dp term uses Clausius-Clapeyron (dTsat/dp) and the superheat sensitivity
of the interfacial heat transfer. When evaporation is inactive (Gamma <= 0 or
subcooled), the augmentation is zero and the diagonal reduces to pure mechanical
compressibility. When evaporation is active, the augmentation grows naturally with
Gamma magnitude -- no tuning parameters, no threshold constants.

Also includes V1 fix: linearized rho_v in void update uses drho_v/dp to estimate
rho_v at new pressure, preventing the void-density positive feedback loop during
...
class BridgeDriftFluxSolver:
    ...
    Variant 4: Augmented diagonal with Gamma-linearized void-density coupling.
    ...
    The pressure diagonal uses phasic mechanical compressibility augmented by
    the Gamma-pressure feedback from Clausius-Clapeyron. This provides smooth
    onset (no 2400x step function) while maintaining correct wave propagation
    speed at established void fractions.
    """
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
        ...
        Args:
            ...
            use_block_coupling: accepted but unused (V4 uses augmented diagonal instead)
            corrector_steps: accepted but unused (V4 does not use predictor-corrector)
        """
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        # Phasic compressibility -- always needed for augmented diagonal
        if self.bridge.has('drho_l_dp') and self.bridge.has('drho_v_dp'):
            ...
            # Fallback: use h_mix drho_dp as both phasic (degenerate case)
            drho_dp_hmix = self.bridge.get('drho_dp')
            drho_l_dp = drho_dp_hmix.copy()
            drho_v_dp = drho_dp_hmix.copy()
        ...
        # and algebraic (sigma->inf) momentum treatment at each face.
        ...
        # -- Pressure tridiagonal (augmented diagonal) --
        # V4: single unified approach for all cells. The diagonal is:
        #   alpha_coeff = V/dt * drho_eff
        # where drho_eff = drho_mech + drho_void_coupling
        #   drho_mech = (1-alpha)*drho_l/dp + alpha*drho_v/dp
        #   drho_void_coupling comes from linearizing the void equation's
        ...
        for i in range(N):
            ...
            # -- Augmented diagonal: phasic mechanical + Gamma-linearized coupling --
            al = alpha_old[i]
            rv_i = max(rho_v[i], 0.01)
            rl_i = max(rho_l[i], 1.0)

            # Phasic mechanical compressibility (~5e-7 for water at 7 MPa)
            drho_mech = (1 - al) * drho_l_dp[i] + al * drho_v_dp[i]

            # Gamma-linearized void-density coupling (RELAP5-like augmentation)
            # Physical derivation:
            #   Void equation: d(alpha)/dt = Gamma/rho_v + flux terms
            #   Linearize Gamma(p) around current state:
            #     dGamma/dp = -Gamma * dTsat/dp / superheat
            #   where dTsat/dp comes from Clausius-Clapeyron:
            #     dTsat/dp = T_sat * (1/rho_v - 1/rho_l) / h_fg
            #   Then dalpha/dp = dt * dGamma/dp / rho_v
            #   And the density change from void shift:
            #     drho_void = (rho_l - rho_v) * dalpha/dp
            #   This is NEGATIVE (pressure drop -> more void -> less density)
            #   For the pressure diagonal: -drho_void is positive (stabilizing)
            drho_void = 0.0
            if Gamma[i] > 0 and T_l[i] > T_sat[i]:
                superheat = max(T_l[i] - T_sat[i], 0.1)
                h_fg = max(h_sat_v[i] - h_sat_l[i], 1.0)

                # Clausius-Clapeyron: dTsat/dp
                dTsat_dp = T_sat[i] * (1.0 / rv_i - 1.0 / rl_i) / h_fg

                # Gamma sensitivity to pressure
                dGamma_dp = -Gamma[i] * dTsat_dp / superheat

                # Void shift from pressure change (over one timestep)
                dalpha_dp = dt * dGamma_dp / rv_i

                # Density change from void shift: (rho_l - rho_v) * dalpha_dp
                # This is negative (dp<0 -> dGamma>0 -> dalpha>0 -> drho<0)
                # We want -drho_void for the diagonal (positive, stabilizing)
                drho_void = (rl_i - rv_i) * dalpha_dp  # negative

            # Effective compressibility: mechanical minus void coupling
            # drho_void is negative, so -drho_void adds to the diagonal
            drho_eff = drho_mech - drho_void

            # Safety: never go below pure mechanical (can't be less compressible
            # than the constituent phases)
            drho_eff = max(drho_eff, drho_mech)
        ...
        # -- Void fraction update (conservative form + V1 linearized rho_v) --
        # Conservative: alpha*rho_v product update, then extract alpha by dividing
        # by linearized rho_v at new pressure. The linearization prevents the
        # void-density positive feedback loop during rapid depressurization:
        #   old rho_v division: dp<0 -> rho_v drops -> alpha overshoots -> more dp<0
        #   linearized rho_v: rho_v_new ~ rho_v_old + drho_v/dp * dp, stays closer
        #   to the actual new-pressure value, damping the feedback.
        for i in range(N):
            al = alpha_old[i]
            rv = max(rho_v[i], 0.01)
            ...
            # V1 fix: linearized rho_v at new pressure
            # rho_v_new ~ rho_v_old + drho_v/dp * (p_new - p_old)
            # This is more accurate than using old rho_v when pressure drops
            # rapidly (e.g., 7 MPa -> 0.1 MPa over the blowdown), where
            # rho_v can change by 60x.
            dp_i = p[i] - p_old[i]
            rv_new = max(rv + drho_v_dp[i] * dp_i, 0.01)
