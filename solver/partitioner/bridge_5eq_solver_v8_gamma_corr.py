"""
bridge_5eq_solver_v8_gamma_corr.py — V8: Gamma correction for new-state superheat.

Keeps the stable base pressure solve (h_mix drho_dp) but corrects Gamma before
the void update using the linearized superheat at the new pressure. When pressure
drops, T_sat drops, superheat increases, and Gamma should be larger. The correction
uses Clausius-Clapeyron (dT_sat/dp) — the same linearization already used in the
existing dGamma/dp diagonal term, but applied to the void update instead of the
pressure diagonal.

This is a NUMERICAL CORRECTION using bridge-provided quantities, not physics
reimplementation. Analogous to the existing semi-implicit friction treatment.
...
class BridgeDriftFluxSolver:
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        # ── V8: Correct Gamma for new-state superheat ──
        # After the pressure solve, pressure has changed. T_sat responds to pressure
        # via Clausius-Clapeyron. The new superheat may activate/enhance Gamma even
        # though the bridge (evaluated at old pressure) showed no/little superheat.
        # This linearized correction breaks the onset delay without changing the
        # pressure equation.
        Gamma_corrected = Gamma.copy()
        for i in range(N):
            dp_i = p[i] - p_old[i]
            if dp_i < 0:  # depressurization
                rv_i = max(rho_v[i], 0.01)
                rl_i = max(rho_l[i], 1.0)
                h_fg = max(h_sat_v[i] - h_sat_l[i], 1.0)
                # Clausius-Clapeyron: dT_sat/dp
                dTsat_dp = T_sat[i] * (1.0 / rv_i - 1.0 / rl_i) / h_fg
                # New-state T_sat and superheat
                T_sat_new = T_sat[i] + dTsat_dp * dp_i
                DT_old = max(T_l[i] - T_sat[i], 0.0)
                DT_new = max(T_l[i] - T_sat_new, 0.0)

                if DT_new > 0 and alpha_old[i] >= 1e-3:
                    if DT_old > 0.01 and Gamma[i] > 0:
                        # Scale existing Gamma by superheat ratio
                        Gamma_corrected[i] = Gamma[i] * DT_new / DT_old
                    elif DT_new > 0.1:
                        # Onset: no old superheat but new superheat exists.
                        # Estimate Gamma from relaxation model using bridge quantities.
                        # H_relax ~ alpha*(1-alpha)*rho_l*cp_f/tau, but we don't have
                        # cp_f or tau here. Use the bridge's q_i_l sensitivity instead:
                        # Gamma = -q_i_l / h_fg, and q_i_l ~ -H_eff * DT
                        # At onset with alpha_nucleation, use the bridge's a_i and h_i.
                        # For now, use a conservative estimate from the existing
                        # semi-implicit diagonal terms.
                        # dGamma/dp = -Gamma * dTsat_dp / DT (from existing code)
                        # But Gamma is zero at onset! We need dGamma/dT_sat instead.
                        # Gamma = H_eff * (T_l - T_sat) / h_fg
                        # dGamma/dT_sat = -H_eff / h_fg
                        # Gamma_new = -H_eff / h_fg * DT_new
                        # H_eff from bridge: q_i_l = -H_eff * DT_old at old state
                        # If DT_old ~ 0, we can't extract H_eff. Skip onset injection.
                        pass
        ...
        if True:
            for i in range(N):
                ...
                alpha_rho_v_new = al * rv + dt / self.V_cell * (flux_v + self.V_cell * Gamma_corrected[i])
                rv_new = max(rho_v[i], 0.01)
                alpha_new = max(0.0, min(1.0, alpha_rho_v_new / rv_new))

                if Gamma_corrected[i] > 0:
