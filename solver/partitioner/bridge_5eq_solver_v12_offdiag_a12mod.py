"""
bridge_5eq_solver_v12_offdiag_a12mod.py -- Variant 12: Combined drift-flux
off-diagonals (V10) + A12 time-constant moderation (V11).

Based on V5 (2x2 block Thomas). Adds two improvements simultaneously:

Improvement 1 (from V10): Drift-flux linearized off-diagonal (1,0) entries.
The vapor mass flux at a face responds to neighbor pressure through the
drift-flux relation:
    d(mdot_v)/dp_neighbor = alpha_face * rho_v_face * beta_eff / rho_face

This gives (1,0) entries in the off-diagonal blocks:
  - a_blocks[i](1,0) = +beta_v_L  (vapor enters from left neighbor)
  - c_blocks[i](1,0) = +beta_v_R  (vapor enters from right neighbor)
  - A21 -= (beta_v_L + beta_v_R)  (vapor leaves this cell through both faces)

Improvement 2 (from V11): Moderate the A12 void-density coupling with a
...
At tau_mix=0 this recovers V5.

The block system per cell is:
    [A11  A12] [delta_p    ]   [R1]
    [A21  A22] [delta_alpha] = [R2]

Row 1: mixture mass conservation (pressure equation) -- A12 moderated
Row 2: vapour mass conservation (void equation) -- with phasic off-diagonals
...
class BridgeDriftFluxSolver:
    """5-equation drift-flux solver with block Thomas, off-diagonals + A12 moderation.

    Combines V10 (drift-flux vapor off-diagonals in block tridiagonal) and V11
    (thermal relaxation moderation of A12 coupling). Physics (properties,
    closures, friction multiplier, interfacial transfer) from OM-generated C.
    Numerics (block tridiagonal solve, momentum, phasic energy) from Python.
    """
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
        ...
        Args:
            ...
            use_block_coupling: accepted for API compatibility but ignored --
                block coupling is always active in V12.
            ...
            tau_mix: thermal relaxation timescale [s] for A12 moderation.
                ...
                Default 5e-4 s. Set to 0 to recover V5 behavior.
        """
        self.reconstruction = reconstruction
        # V12: block coupling is always active; corrector is unnecessary
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        V12 combines drift-flux off-diagonals (vapor flux coupling to neighbor
        pressure) with A12 thermal relaxation moderation. The block solve
        couples pressure and void fraction implicitly via mixture mass (Row 1)
        and vapour mass (Row 2) conservation.
        ...
        # Face-centered drift-flux quantities for off-diagonal vapor coupling
        # alpha_face[i] and rho_v_face[i] are at face i (between cell i-1 and i)
        if self.bridge.has('alpha_face') and self.bridge.has('rho_v_face'):
            alpha_face = self.bridge.get('alpha_face')
            rho_v_face = self.bridge.get('rho_v_face')
        else:
            # Fallback: simple averaging from cell-centered values
            alpha_face = np.zeros(N + 1)
            rho_v_face = np.zeros(N + 1)
            for i in range(N + 1):
                if i == 0:
                    alpha_face[i] = alpha_old[0]
                    rho_v_face[i] = rho_v[0]
                elif i == N:
                    alpha_face[i] = alpha_old[N - 1]
                    rho_v_face[i] = rho_v[N - 1]
                else:
                    alpha_face[i] = 0.5 * (alpha_old[i - 1] + alpha_old[i])
        ...
        #   [A21  A22] [delta_alpha] = [R2]
        #
        # Row 1 (mixture mass): V/dt * drho_mech * dp + V/dt * (rho_v - rho_l)/(1+tau/dt) * dalpha
        ...
        #   + drift-flux off-diagonal coupling to neighbor pressures
        ...
        for i in range(N):
            ...
            # ---- Drift-flux vapor coupling coefficients (V10 improvement) ----
            # Vapor mass flux response to neighbor pressure change:
            #   beta_v = alpha_face * rho_v_face * beta_eff / rho_face
            # Left face (face i): couples cell i to cell i-1
            if self.inlet_closed and i == 0:
                beta_v_L = 0.0  # closed wall -- no vapor flux at inlet
            elif i == 0:
                beta_v_L = 0.0
            else:
                beta_v_L = (alpha_face[i] * rho_v_face[i] * beta_eff[i]
                            / max(rho_face[i], 1.0))

            # Right face (face i+1): couples cell i to cell i+1
            if i == N - 1 and outlet_choked:
                beta_v_R = 0.0  # choked outlet -- vapor flux fixed
            else:
                beta_v_R = (alpha_face[i + 1] * rho_v_face[i + 1] * beta_eff[i + 1]
                            / max(rho_face[i + 1], 1.0))
            ...
            # Moderated by thermal relaxation timescale tau_mix (V11 improvement).
            ...
            # Drift-flux off-diagonal contribution to diagonal (V10 improvement):
            # Vapor flux leaving this cell through both faces when THIS cell's
            # pressure changes. This is the (1,0) diagonal contribution.
            ...
            # Off-diagonal blocks: pressure coupling (0,0) + vapor flux coupling (1,0)
            # (0,0): mixture momentum couples pressure to neighbor pressure
            # (1,0): drift-flux vapor flux response to neighbor pressure change
            if i > 0:
