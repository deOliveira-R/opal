"""
bridge_5eq_solver_v16_energy_sub.py -- Variant 16: Energy sub-stepping after block solve.

Based on V11 (2x2 block Thomas with A12 moderation). V11 updates p and alpha
implicitly via the block solve, but the phasic enthalpies (h_l, h_v) update
explicitly AFTER the block solve using old-state properties. During rapid
flashing this causes:

1. Block solve: pressure drops, void grows correctly
2. Energy update: uses old Gamma, old q_i_l -> h_l doesn't cool fast enough
3. Next step: bridge sees artificially high h_l -> superheat too large
   -> Gamma too high -> void overshoots

V16 fix: after the block solve + momentum update, re-evaluate the bridge at
the new (p, alpha, mdot) state with old enthalpies, then perform the energy
update with the refreshed source terms. This costs one extra bridge evaluation
per step (2x total), but the energy equation sees:

- T_sat(p_new) which is LOWER than T_sat(p_old) -> larger superheat -> larger Gamma
- Correct alpha from block solve -> correct phase masses
- h_sat values at new pressure -> correct enthalpy limiters

This reduces void overshoot because the energy update cools h_l faster (larger
Gamma at new state), h_l approaches h_f(p_new) rather than staying artificially
high, and the next step sees lower superheat -> less Gamma -> void self-limits.

The block system per cell is (same as V11):
    [A11  A12] [delta_p    ]   [R1]
...
class BridgeDriftFluxSolver:
    """5-equation drift-flux solver: block Thomas + energy sub-stepping.

    Based on V11 (block Thomas with A12 moderation). Adds energy sub-stepping:
    after the block solve updates p and alpha, and after momentum update,
    re-evaluates the bridge at the new (p, alpha, mdot) state before performing
    the phasic energy update. This gives the energy equation access to:

    - Updated Gamma at the new superheat (T_l_old vs T_sat(p_new))
    - Updated q_i_l, q_i_v at new conditions
    - Updated h_sat_l, h_sat_v for enthalpy limiters
    - Updated rho_l, rho_v for energy equation mass terms
    - Updated phasic mass flows mdot_v, mdot_l at new state

    Cost: 2 bridge evaluations per step (same as V2/V7).
    Benefit: energy update sees correct source terms, reducing void overshoot.
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        # ====================================================================
        # V16: RE-EVALUATE BRIDGE AT NEW (p, alpha, mdot) FOR ENERGY UPDATE
        # ====================================================================
        # The block solve changed p and alpha; momentum update changed mdot.
        # Re-evaluating the bridge at the new state gives updated source terms
        # for the energy equation:
        # - Gamma at new superheat (T_l_old vs T_sat(p_new)) -- larger when
        #   p drops because T_sat drops while T_l hasn't cooled yet
        # - q_i_l, q_i_v at new interfacial conditions
        # - h_sat_l, h_sat_v at new pressure for enthalpy limiters
        # - rho_l, rho_v at new state for phase mass terms
        # - mdot_v, mdot_l phasic mass flows at new state (if drift flux)
        #
        # Enthalpies are still h_l_old, h_v_old -- they haven't been updated yet.
        # This is correct: the bridge computes Gamma from (p_new, alpha_new,
        # h_l_old, h_v_old), which represents the actual thermodynamic state
        # entering the energy update.
        self.bridge.set_state(p, alpha=alpha, h_l=h_l_old, h_v=h_v_old, mdot=mdot)
        self.bridge.evaluate()

        # Refresh all variables used by the energy update
        Gamma = self.bridge.get('Gamma')
        q_i_l = self.bridge.get('q_i_l')
        q_i_v = self.bridge.get('q_i_v')
        rho_l = self.bridge.get('rho_l')
