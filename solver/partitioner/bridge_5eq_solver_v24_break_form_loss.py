"""
bridge_5eq_solver_v24_break_form_loss.py -- Variant 24: Break form-loss coupling.

Based on V11 (2x2 block Thomas + A12 moderation + isentropic A11).
Adds a localized form loss at the outlet face that enforces the C_d ramp
through the momentum equation, not just through the critical flow clamp.

Problem: For subcooled liquid at N_param=0, the Henry-Fauske critical flow
model gives Bernoulli discharge ~400+ kg/s. The momentum equation builds
mdot incrementally and never reaches this limit. The min() clamp never binds.
The break opens instantaneously as far as the momentum equation sees.

Fix: K_break = (1/C_d^2 - 1) at the outlet face. This enters the implicit
friction treatment identically to Darcy friction. When C_d → 0: K_break → ∞
(break closed). When C_d = 1: K_break = 0 (no extra resistance).
