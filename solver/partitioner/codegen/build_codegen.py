...
def make_opal_name(omc_name: str) -> str:
    """Convert omc_library_Media_SimpleFluid_rho__ph -> opal_rho_ph.

    Strip the package prefix and replace __ with _.
    """
    # Remove omc_library_Media_{MediumName}_ prefix
    m = re.match(r'omc_library_Media_\w+?_(.*)', omc_name)
    if m:
        short = m.group(1)
    else:
        # Fallback: just strip omc_
...
def generate_standalone_c(functions_c: Path, functions_h: Path,
    ...
    for f in funcs:
        opal_name = make_opal_name(f['name'])
