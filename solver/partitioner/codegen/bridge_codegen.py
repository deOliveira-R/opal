...
def generate_bridge_c(info: ModelInfo, model_c: Path, functions_c: Path,
    ...
    lines.append('static int _opal_error_count = 0;')
    lines.append('static void throwStreamPrint(threadData_t *td, const char *fmt, ...) {')
    lines.append('    (void)td;')
    lines.append('    if (_opal_error_count < 5) {')
    lines.append('        va_list args; va_start(args, fmt);')
    lines.append('        fprintf(stderr, "OPAL bridge warning: "); vfprintf(stderr, fmt, args);')
    lines.append('        fprintf(stderr, "\\n"); va_end(args);')
    lines.append('    }')
    lines.append('    _opal_error_count++;')
    lines.append('}')
    lines.append('')
    ...
    # OM comparison operator functions (generated by OM for conditional expressions
    # that bypass relationhysteresis — e.g., parameter-level >= comparisons)
    lines.append('/* Bare comparison functions generated by OM */')
    lines.append('#define GreaterEq(a, b) ((a) >= (b))')
    lines.append('#define Greater(a, b)   ((a) > (b))')
    lines.append('#define LessEq(a, b)    ((a) <= (b))')
    lines.append('#define Less(a, b)      ((a) < (b))')
    lines.append('')

    # OM runtime utility stubs (used by IAPWS and complex media models)
    lines.append('/* Integer power: x^n for integer n (used in IAPWS Gibbs functions) */')
    lines.append('static inline modelica_real real_int_pow(threadData_t *td, modelica_real x, modelica_integer n) {')
    lines.append('    (void)td;')
    lines.append('    return pow(x, (double)n);')
    lines.append('}')
    lines.append('')
    lines.append('/* Range check for integer for-loops */')
    lines.append('static inline modelica_boolean in_range_integer(modelica_integer i,')
    lines.append('    modelica_integer start, modelica_integer stop) {')
    lines.append('    return (start <= stop) ? (i >= start && i <= stop)')
    lines.append('                           : (i >= stop && i <= start);')
    lines.append('}')
    lines.append('')
    ...
    lines.append('    (void)td; (void)info; (void)msg; /* assertion ignored — solver clamps inputs */ }')
    ...
    lines.append('    (void)td; (void)info; (void)idx;')
    lines.append('    if (_opal_error_count < 5) {')
    lines.append('        va_list args; va_start(args, msg);')
    lines.append('        fprintf(stderr, "OPAL bridge eq warning: "); vfprintf(stderr, msg, args);')
    lines.append('        fprintf(stderr, "\\n"); va_end(args);')
    lines.append('    }')
    lines.append('    _opal_error_count++;')
    lines.append('}')
