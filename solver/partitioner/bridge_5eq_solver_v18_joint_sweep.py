"""
bridge_5eq_solver_v18_joint_sweep.py -- Variant 18: Joint tau_flash + tau_mix sweep.

Based on V11 (2x2 block Thomas with A12 time-constant moderation). Adds
tau_flash_override to the constructor, enabling a 2D parameter sweep from the
comparison harness over both numerical (tau_mix) and physical (tau_flash) timescales.

V11's tau_mix controls the numerical pressure-void coupling. The Modelica model's
tau_flash controls the physical flashing rate. These interact:
- tau_flash was tuned to 0.025 under the stalled regime (140ms onset) where it had
  negligible effect.
- With V11's correct onset (10ms), tau_flash directly controls void growth speed.
- The optimal tau_flash under V11 may be very different from 0.025.

Usage from comparison harness:
    for tau_mix in [2e-4, 3e-4, 4e-4, 5e-4]:
        for tau_flash in [0.001, 0.005, 0.01, 0.025, 0.05]:
            solver = BridgeDriftFluxSolver(bridge, spec, es=es,
                                           tau_mix=tau_mix,
                                           tau_flash_override=tau_flash)
...
class BridgeDriftFluxSolver:
    ...
    and configurable tau_flash override for joint parameter sweeps.

    Based on V11 block Thomas. Adds tau_flash_override to enable 2D sweeps
    over both the numerical coupling timescale (tau_mix) and the physical
    flashing timescale (tau_flash) from the comparison harness.
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
                 ...
                 corrector_steps=0, tau_mix=5e-4, tau_flash_override=None,
                 **kwargs):
        """
        Args:
            ...
            use_block_coupling: accepted for API compatibility but ignored --
                block coupling is always active in V11.
            ...
            tau_flash_override: if set, override the Modelica model's tau_flash
                parameter in the bridge after set_params_from_spec. This enables
                sweeping the physical flashing timescale without recompiling the
                Modelica model. None means use the value from the Modelica model.
            **kwargs: absorbed for compatibility with comparison harness.
        """
        self.reconstruction = reconstruction
        # Block coupling is always active; corrector is unnecessary
        self.use_block_coupling = True
        self.corrector_steps = 0
        self.tau_mix = tau_mix
        self.tau_flash_override = tau_flash_override
        self.bridge = bridge
        self.N = bridge.N
        self.spec = spec
        ...
        # V18: override tau_flash in bridge parameter array after set_params_from_spec
        if tau_flash_override is not None:
            self._override_bridge_param(
                f'{bridge.prefix}.tau_flash', tau_flash_override)

    def _override_bridge_param(self, param_name, value):
        """Override a single parameter in the bridge after set_params_from_spec.

        Uses the bridge's opal_bridge_set_param (singular) to write one parameter
        by index. This avoids rebuilding the entire parameter array and is safe
        to call after set_params_from_spec has already written all parameters.

        Args:
            param_name: full parameter name (e.g., 'pipe.tau_flash')
            value: new parameter value (float)

        Raises:
            KeyError: if param_name is not found in the bridge info
        """
        if param_name not in self.bridge.info.parameters:
            raise KeyError(
                f"Parameter '{param_name}' not found in bridge. "
                f"Available parameters with 'tau': "
                f"{[k for k in self.bridge.info.parameters if 'tau' in k]}")
        pinfo = self.bridge.info.parameters[param_name]
        self.bridge.lib.opal_bridge_set_param(pinfo.index, float(value))
