"""
bridge_5eq_solver_v6_block_blend.py -- Variant 6: Block Thomas + alpha-blended compressibility.

Combines V5's 2x2 block Thomas (correct void onset at 15ms) with alpha-dependent
h_mix compressibility blending (correct wave propagation). At small alpha (onset),
A11 uses phasic drho_dp for rapid nucleation. As alpha grows, A11 transitions to
h_mix drho_dp for correct wave speed — the thermal compressibility naturally enters
through the mixture EOS rather than as a step function.
This is the RELAP5 approach: mixture mass conservation and vapour mass
conservation are solved simultaneously, eliminating the pressure-void lag.
...
class BridgeDriftFluxSolver:
    ...
    def __init__(self, bridge: OMEquationBridge, spec, es=None,
        ...
        Args:
            ...
            reconstruction: 'donor_cell' or 'muscl'
            alpha_onset: void fraction below which phasic drho_dp is used (onset regime)
            alpha_wave: void fraction above which h_mix drho_dp is used (wave regime)
        """
    ...
    def step(self, p, alpha, h_l, h_v, mdot, dt):
        ...
        for i in range(N):
            ...
            # V6: Blend phasic mechanical compressibility with h_mix thermal
            # compressibility based on alpha. At alpha<ALPHA_ONSET, use phasic
            # (allows rapid nucleation/onset). At alpha>ALPHA_WAVE, use h_mix
            # (correct wave propagation speed). Smooth sigmoid transition.
            ...
            if al <= ao:
                ...
                blend = (al - ao) / (aw - ao)
                drho_A11 = (1 - blend) * drho_mech + blend * drho_dp[i]
            A11 = self.V_cell / dt * drho_A11 + bL + bR
            # Void changes mixture density: d(rho_m)/d(alpha) = rho_v - rho_l
            # V6: Damp A12 at small alpha to prevent massive pressure feedback
            # from tiny void nucleation. Full coupling at alpha > alpha_wave.
            a12_damp = min(al / max(aw, 1e-6), 1.0)
            A12 = self.V_cell / dt * (rv_i - rl_i) * a12_damp
        ...
        for i in range(N):
            ...
            # Update void fraction — cap delta_alpha per step for stability
            MAX_DALPHA = 0.05  # max void change per timestep
            delta_alpha = max(-MAX_DALPHA, min(MAX_DALPHA, delta_alpha))
