...
class TestBoundaryMapping:
    ...
    def test_closed_inlet_gives_wallface(self):
        ...
        bc_in, bc_out = boundary_faces_from_spec(spec, 700e3, 2800e3)
        # WallFace should enforce zero flow (no pressure method)
        assert bc_in is not None
    ...
    def test_break_outlet_gives_breakface(self):
        spec = Pipe1DGridSpec(
            ...
            inlet_closed=True, outlet_closed=False,
            p0=[10e6]*3, h0=[700e3]*3, mdot0=[0]*4)
        bc_in, bc_out = boundary_faces_from_spec(spec, 700e3, 2800e3, C_d=0.87)
        # BreakFace should have critical flow capability
        assert bc_out is not None
...
class TestSignConventions:
    ...
    def test_port_b_sign_convention(self):
        """Pipe1D.mo: mdot[N+1] = -port_b.m_flow.
        Positive port_b.m_flow = flow INTO component (from right).
        Positive mdot[N+1] = flow LEFT to RIGHT (out of component).
        So mdot[N+1] = -port_b.m_flow is correct."""
        # This is a documentation/structural test
        # The round-trip tests (Gap 1) verify this numerically
        assert True
