...
model Pipe1D_DriftFlux_RegimeHT
  "1D pipe with 5-equation drift-flux — regime-dependent IAC+HTC hardcoded (Ishii-Mishima 1984)"
  ...
  // Regime-dependent IAC+HTC is HARDCODED in this model (no parameter switch).
  // Bubbly (alpha < 0.3) → Annular (alpha > 0.5) with smooth blend.
...
equation
  ...
  for i in 1:N loop
    ...
    // Regime-dependent IAC (hardcoded): bubbly 6α/d_b → annular 4√α/D_h
    a_i[i] = (1.0 - blend_regime[i])
               * 6.0 * alpha_eff[i] / max(d_b_eff[i], d_b_min)
             + blend_regime[i]
               * 4.0 * sqrt(noEvent(max(alpha_eff[i], 0.01))) / D_h;

    // Regime-dependent HTC (hardcoded): bubbly Nu*k_f/d_b → annular k_f/δ_film
    h_i[i] = (1.0 - blend_regime[i])
